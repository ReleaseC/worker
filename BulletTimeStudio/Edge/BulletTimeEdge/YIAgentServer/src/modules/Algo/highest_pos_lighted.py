...
def parse_args():
    ...
    parser.add_argument('--wVideo', dest='wVideo', help='Width of the video',
                        default=1920)
    parser.add_argument('--noOutFile', dest='noOutFile', help='to close frame and json save', 
                        default='')
...
for num in range(0, 20000):
    ...
    for c in cnts:
        ...
        if int(args.noOutFile):
            if not os.path.exists(args.output_dir):
                os.makedirs(args.output_dir)

            cv2.imwrite(os.path.join(args.output_dir, str(num) + '.jpg'), frame[:,:,::-1])
