...
for i in range(0, n_images):
    ...
    if not int(args.calibrated):
        im = Image.fromarray(src[:,:,::-1]) 
        im.save(args.dataDirOutput + '%03d_adjust0.jpg' % (i + 1) ,"JPEG")
        print ';fileName[',i,']=',args.dataDirOutput + '%03d_adjust0.jpg' % (i+1),';'
...
if int(args.calibrated):
    ...
    for idx, row in enumerate(xy_real):
        src = cv2.imread(args.dataDirInput + '%d.jpg' % (idx + 1), cv2.IMREAD_COLOR)
        xy_all_real = []
        xy_all_interpolated = []
        for j in range(0, 4):
        ...
        im = Image.fromarray(src[:,:,::-1])
        im.save(args.dataDirOutput + '%03d_adjust1.jpg' % (idx + 1) ,"JPEG")
        print 'xy_all_real[',idx,']=',xy_all_real,';xy_all_interpolated[',idx,']=',xy_all_interpolated,';fileName[',idx,']=',args.dataDirOutput + '%03d_adjust1.jpg' % (idx+1),';'