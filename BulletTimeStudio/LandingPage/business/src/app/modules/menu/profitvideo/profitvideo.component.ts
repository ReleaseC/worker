...
@Component({
    selector: 'ngx-profitvideo',
    styleUrls: ['./profitvideo.component.css'],
    // styleUrls: ['./profitvideo.component.css'],
    templateUrl: './profitvideo.component.html',

})
export class ProfitVideoComponent implements OnInit {
    date = null; // new Date();
    ...
    getActivityList() {
        ...
        this.http.get(environment.apiServer + '/activity/list?project_id=' + this.project_id).subscribe(
            res => {
    ...
    remark(order_id):void{
        var remark = prompt("您的退款理由是：")
        if(remark!=null && remark!=""){
            this.toggleFlag(order_id,remark)
        //   console.log('点击确定了',remark)
        }else{
            alert('请填写退款原因')
        }
    }
    toggleFlag(order_id,remark):void {
        ...
        let params = {order_id:order_id, refund: refund,mark:remark};
