...
export class PanelComponent implements OnInit {
  ...
  calIndexToTime(type, index) {
    const time = (type === 'main') ? index : this.currentThumbnailIndex;
    const hour = Math.floor(time / 3600);
    const min = Math.floor((time % 3600) / 60);
    const sec = (time % 3600) % 60;
    return (type === 'main') ?
            ('0' + hour).slice(-2) + ':' + ('0' + min).slice(-2) + ':' + ('0' + sec).slice(-2) :
            ('0' + hour).slice(-2) + ':' + ('0' + min).slice(-2) + ':' + ('0' + sec).slice(-2) + '_' + index;
  }
          ...
          if (dataLen) {
            for (let i = 0; i < dataLen; i++) {
              this.taskArr.unshift({
                ...
                'mainIndex': Math.floor(data['result'][i].task.frame / 30),
                'childIndex': data['result'][i].task.frame % 30,
                'goalType': data['result'][i].task.goalType,
                'activityType': data['result'][i].task.type,
                'isTaskCreated': true
              });
    ...
    this.http
        ...
        .subscribe(data => {
          if (data['code'] === 0) {
            ...
            this.socketService.requestFrameBySiteId({'activityId': this.siteId});
  ...
  sendGoalEvent(index) {
    ...
    // Send task to server
    this.http.post(this.mediaServer + 'api/create_task', body)
        .subscribe(
            ret => {
  ...
  Goal() {
    ...
    this.taskArr.unshift({
      ...
      'frame': this.currentThumbnailIndex * 30 + this.currentChildThumbnailIndex,
