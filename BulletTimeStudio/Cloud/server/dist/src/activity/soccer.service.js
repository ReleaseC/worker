...
let SoccerService = class SoccerService {
    constructor() {
        this.connecntMap = new Map();
        this.Promise = require('bluebird');
        this.client = this.Promise.promisifyAll(redis.createClient());
        this.goalMap = new Object();
        this.deviceIdSocketIdMap = new Object();
    }
    initService(io, socket) {
        this.tempSocket = io;
        socket.on(socket_interface_1.SOCCER_EVENT.SITES_MATCH_EVENT, (data, cb) => __awaiter(this, void 0, void 0, function* () {
            ...
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.SITES_CONFIG_EVENT, soccerSite.deviceConfig);
            let temp = [];
            let isExist = false;
        ...
        socket.on(socket_interface_1.SOCCER_EVENT.SITES_UNMATCH_EVENT, (data) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (isDeviceBiding) {
                ...
                io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.SITES_CONFIG_EVENT, isDeviceBiding.deviceConfig);
            }
        }));
        socket.on(socket_interface_1.SOCCER_EVENT.SITES_CONFIG_EVENT, (data) => __awaiter(this, void 0, void 0, function* () {
            console.log('site=' + JSON.stringify(data));
            const soccerSite = yield db_service_1.soccorDbSiteSetting.findOne({ siteId: data.siteId });
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.SITES_CONFIG_EVENT, soccerSite.deviceConfig);
        }));
        socket.on(socket_interface_1.SOCCER_EVENT.GOAL_EVENT, (data) => __awaiter(this, void 0, void 0, function* () {
            ...
            socket.emit(socket_interface_1.SOCCER_EVENT.EVENT_GOAL_SUCCESS_ACK, res);
            socket.broadcast.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.PREPARE_UPLOAD_EVENT, res);
        ...
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_CAM_GOAL_ACK, (data) => __awaiter(this, void 0, void 0, function* () {
        ...
        socket.on(socket_interface_1.SOCCER_EVENT.SINGLE_VIDEO_UPLOADED_EVENT, (data) => {
            console.log('Single video uploaded goal data=' + JSON.stringify(data));
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.SINGLE_VIDEO_UPLOADED_EVENT, data);
            let taskService = new task_service_1.TaskService();
            if (taskService.isAllFilesUpdated(data.taskId)) {
                this.soccerUpdateTask();
            }
        });
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT_GETDEVICES, (data) => {
            console.log('remote shot get devices data=' + JSON.stringify(data));
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT_GETDEVICES, data);
        });
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT, (data) => {
            console.log('remote shot data=' + JSON.stringify(data));
            socket.join(data.siteId);
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT, data);
        });
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT_GET_PICS, (data) => {
            ...
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.EVENT_REMOTE_SHOT_GET_PICS, data);
        });
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_HEARTBEAT, (data) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (data.role == 'admin') {
                ...
                socket.broadcast.emit(socket_interface_1.SOCCER_EVENT.EVENT_HEARTBEAT, data);
            }
            else {
                let site = yield db_service_1.soccorDbSiteSetting.findOne({ siteId: data.siteId });
                if (site && site.deviceConfig) {
        ...
        socket.on(socket_interface_1.SOCKET_EVENT.DISCONNECT, (data, cb) => __awaiter(this, void 0, void 0, function* () {
            if (this.connecntMap.get(socket) != undefined) {
                for (let i = 0; i < this.connecntMap.get(socket).length; i++) {
                    socket.broadcast.emit(socket_interface_1.SOCCER_EVENT.EVENT_DEVICEOFFLINE, this.connecntMap.get(socket)[i]);
                }
            }
        }));
        socket.on(socket_interface_1.SOCCER_EVENT.EVENT_SOCCER_CMD, (data) => {
            console.log('cmd=' + JSON.stringify(data));
            io.to(data.siteId).emit(socket_interface_1.SOCCER_EVENT.EVENT_SOCCER_CMD, data);
        });
    }
    soccerUpdateTask() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < retArrLen; i++) {
                let retOfFiles = yield taskService.get_task_file_lists({ taskId: retArr[i].task.taskId });
        ...
        return __awaiter(this, void 0, void 0, function* () {
            for (var taskId in this.goalMap) {
                ...
                if (res.deviceId_0.length > 0 && this.deviceIdSocketIdMap[res.deviceId_0]) {
                    this.tempSocket.to(this.deviceIdSocketIdMap[res.deviceId_0]).emit(socket_interface_1.SOCCER_EVENT.PREPARE_UPLOAD_EVENT, this.goalMap[taskId]);
                }
                if (res.deviceId_1.length > 0 && this.deviceIdSocketIdMap[res.deviceId_1]) {
                    this.tempSocket.to(this.deviceIdSocketIdMap[res.deviceId_1]).emit(socket_interface_1.SOCCER_EVENT.PREPARE_UPLOAD_EVENT, this.goalMap[taskId]);
                }
                if (res.deviceId_2.length > 0 && this.deviceIdSocketIdMap[res.deviceId_2]) {
                    this.tempSocket.to(this.deviceIdSocketIdMap[res.deviceId_2]).emit(socket_interface_1.SOCCER_EVENT.PREPARE_UPLOAD_EVENT, this.goalMap[taskId]);
                }
            }
        });
    }
