import {HttpClientBuckets, HttpClientCallback, HttpClientListOptions} from "./httpclient.interface";
...
class HttpClient {
     ...
     */
    public async list(options: HttpClientListOptions, callback: HttpClientCallback, bucket?: HttpClientBuckets) {
     ...
     */
    public async get(originFileName: string, localFileName: string, callback: HttpClientCallback,
               times?: number, bucket?: HttpClientBuckets) {
        ...
        await this.client.get(originFileName, localFileName)
            .then(callback.onSuccess)
            .catch(err => {
                if (times-- > 0) {
