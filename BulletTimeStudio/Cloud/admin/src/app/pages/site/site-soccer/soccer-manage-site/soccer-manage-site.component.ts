...
@Component({
  selector: 'ngx-soccer-manage-site',
  templateUrl: './soccer-manage-site.component.html',
  styleUrls: ['./soccer-manage-site.component.scss'],
})
export class SoccerManageSiteComponent implements OnInit {
  originSiteListArr = [];
  siteListArr = [];
  // ToDo: siteNameArr = [];
  options = '';
  ...
  ngOnInit() {
    ...
    //     .deviceDisconnect()
    //       .subscribe(data => {
    //         console.log('deviceDisconnect=' + JSON.stringify(data));
    //         for (let i = 0; i < this.siteListArr.length; i++) {
    //           if ((this.siteListArr[i].siteId === data['siteId']) &&
    //             (this.siteListArr[i].deviceId === data['deviceId']) &&
    //             (this.siteListArr[i].role === data['role'])) {
    //             this.siteListArr[i].status = 'offline';
    //             this.siteListArr[i].statusColor = 'gray';
    //             this.siteListArr[i].lastUpdate = '';
    //             break;
    //           }
    //         }
    //       });

    // ToDo: this.readRadioValue(this.radioValue);
